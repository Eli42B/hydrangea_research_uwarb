...
dmerge7 = dmerge %>% 
  ...
  select(Cultivar, Species, shannon)
print(dmerge7, n = 25)
...
plot(dmerge$shannon ~ dmerge$TotalInflorescences_AB, xlab = "Natural Log Inflorescence Count",ylab = "Daily Insect Diversity", main = "Plants with More Inflorescences Were Associated \nwith More Diverse Assemblages of Insects")
lm = abline(lm(dmerge$shannon ~ dmerge$TotalInflorescences_AB), col = "red") #Add a linear model and line 
summary(lm(dmerge$TotalInsects ~ dmerge$TotalInflorescences_AB))
text(1.6,1.6, "Insect Diversity = 1.106(logx) - 2.387
     p < 0.0001***
     R-squared: 0.195",
     cex = 0.7, pos = 3)
