...
sum(dmerge$TotalInsects)
dmerge_bumbles = dmerge[dmerge$Bombus.affinis]
...
#Figuring out total relative abundance of all bugs per plant per season 
...
#colnames(dmerge) = paste0("5","50","400","3,000","22,000","150,000")


plot(dmerge$TotalInsects ~ dmerge$TotalInflorescences_AB, xlab = "Inflorescence Count",ylab = "Insects Per Day", main = "Plants with More Inflorescences Attracted More Insects Per Day")
lm = abline(lm(dmerge$TotalInsects ~ dmerge$TotalInflorescences_AB), col = "red") #Add a linear model and line 
summary(lm(dmerge$TotalInsects ~ dmerge$TotalInflorescences_AB))
text(3.5,23, "Insects per day = 1.465x - 5.445
     p < 0.001***
     R-squared: 0.176",
     cex = 0.7, pos = 3)
...
#first model with species and interaction but reversed 
model1 = glm(TotalInsects ~ TotalInflorescences_AB*Species + TempF+ Month, data = dmerge, family = quasipoisson(link = "log"))
p = interact_plot(model1, pred = TotalInflorescences_AB, modx = Species)
p + 
  labs(title = "Both Hydrangea Species Attracted More Insects\nAs Inflorescences Increased, but at Different Rates",
       x = "Inflorescences", 
       y = "Average Insects Per Plant Per Day", 
       fill = "Total Inflorescences per Plant")
# It's close, but species is not actually interacting with inflorescence count 

#-----------------2nd model with flower type and interaction
...
#2nd model with species and interaction but reversed 
model2 = glm(TotalInsects ~ TotalInflorescences_AB*Flower.Type + TempF+ Month, data = dmerge, family = quasipoisson(link = "log"))
...
#3rd model with fertile flower ratios and interaction but reversed 
model3 = glm(TotalInsects ~ TotalInflorescences_AB*Inflorescence.Type + TempF+ Month, data = dmerge, family = quasipoisson(link = "log"))
p = interact_plot(model3, pred = TotalInflorescences_AB, modx = Inflorescence.Type)
p + 
  labs(title = "All Inflorescence Types Attracted More Insects\nAs Inflorescences Increased, but at Different Rates",
       x = "Inflorescences", 
       y = "Average Insects Per Plant Per Day")
...
#Simplified flower type model 
model = glm(TotalInsects ~ TotalInflorescences_AB*Flower.Type + Month, data = dmerge, family = quasipoisson(link = "log"))
summary(model)
...
model = glm(TotalInsects ~ TotalInflorescences_AB*Species + Month, data = dmerge, family = quasipoisson(link = "log"))
summary(model)
...
dAgg2$TotalBugs = rowSums(dAgg2[,2:6])
dAgg2
