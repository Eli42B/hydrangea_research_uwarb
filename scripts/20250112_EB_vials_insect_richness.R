...
plot(dmerge$InsectRichness ~ dmerge$TotalInflorescences_AB, xlab = "Inflorescence Count",ylab = "Insect Richness Per Day", main = "Plants with More Inflorescences \n Attracted Higher Number of Unique Species Per Day")
lm = abline(lm(dmerge$InsectRichness ~ dmerge$TotalInflorescences_AB), col = "red") #Add a linear model and line 
summary(lm(dmerge$InsectRichness ~ dmerge$TotalInflorescences_AB))
text(4,9, "Insect Richness per day = 0.633x - 1.49
     p < 0.0001***
     R-squared: 0.170",
     cex = 0.7, pos = 3)
...
#Simplified model (Flower Type, month) 
model = glm(InsectRichness ~ TotalInflorescences_AB*Flower.Type + Month, data = dmerge, family = quasipoisson(link = "log"))
summary(model)
...
#simplified model with species and no interaction 
model3 = glm(InsectRichness ~ TotalInflorescences_AB + Species + Month, data = dmerge, family = quasipoisson(link = "log"))
summary(model3)
