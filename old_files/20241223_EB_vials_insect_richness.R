...
p = ggplot(data = dmerge, 
  ...
  labs(x = "Hydrangea Flower Type", 
       y = "Daily Insect Richness",
       title = "Hydrangea Flower Type Impact on Pollinator Richness",
...
dmerge = dmerge[dmerge$TotalInflorescences_AB != "-Inf",]
dmerge$TotalInflorescences_AB = dmerge[is.finite(rowSums(dmerge$TotalInflorescences_AB)),]


df <- df[is.finite(rowSums(df)),]

#@ now subset it!!
data<-data[data$Type=="Guests",]
...
model = glm(InsectRichness ~ TotalInflorescences_AB + Wind + Rain + Cloud + TempF + Month + Cultivar + Species, data = dmerge, family = quasipoisson(link = "log"))
summary(model)
#OK we need to do forwards regression, too many variables, hard to interpret 

model = glm(InsectRichness ~ TotalInflorescences_AB, data = dmerge, family = quasipoisson(link = "log"))
summary(model)
#Total Inflorescences p value = 1.3E-5 < 0.001
#inflorescences matter, we can keep for now 

model = glm(InsectRichness ~ TotalInflorescences_AB + Wind, data = dmerge, family = quasipoisson(link = "log"))
summary(model)
#Wind p value = 0.806, let's drop wind 

model = glm(InsectRichness ~ TotalInflorescences_AB + Rain, data = dmerge, family = quasipoisson(link = "log"))
summary(model)
...
poisson.model = glm(InsectRichness ~ TotalInflorescences_AB + Cloud, data = dmerge, family = poisson(link = "log"))
summary(poisson.model)
#Cloud p value = 0.979, let's drop cloud 

#TotalInsects ~ 1.12*TempF + -0.0082(TempF^2) - 31.70
poisson.model = glm(InsectRichness ~ TotalInflorescences_AB + TempF, data = dmerge, family = poisson(link = "log"))
summary(poisson.model)
#TempF p value = >> 0.05, let's drop temp  

poisson.model = glm(InsectRichness ~ TotalInflorescences_AB + Month, data = dmerge, family = poisson(link = "log"))
summary(poisson.model)
#Month p value = 0.005, p < 0.01, let's keep 

poisson.model = glm(InsectRichness ~ TotalInflorescences_AB + Species, data = dmerge, family = poisson(link = "log"))
summary(poisson.model)
#Species p value = 4.19E-9, p < 0.001, let's keep 

#OK so now let's go forwards 
model = glm(InsectRichness ~ TotalInflorescences_AB, data = dmerge, family = quasipoisson(link = "log"))
summary(model)
model = glm(InsectRichness ~ TotalInflorescences_AB + TempF, data = dmerge, family = quasipoisson(link = "log"))
...
model = glm(InsectRichness ~ TotalInflorescences_AB + TempF+ Month, data = dmerge, family = quasipoisson(link = "log"))
summary(model) #inflorescences and month are looking significant, drop temp 

#first model with species 
model1 = glm(InsectRichness ~ TotalInflorescences_AB + Month + Species, data = dmerge, family = quasipoisson(link = "log"))
...
model2 = glm(InsectRichness ~ TotalInflorescences_AB + Month + Flower.Type, data = dmerge, family = quasipoisson(link = "log"))
summary(model2) 
...
model3 = glm(InsectRichness ~ TotalInflorescences_AB + Month + Inflorescence.Type, data = dmerge, family = quasipoisson(link = "log"))
summary(model3)
interact_plot(model3, pred = Inflorescence.Type, modx = TotalInflorescences_AB)

#first model with species and interaction 
model1 = glm(InsectRichness ~ TotalInflorescences_AB*Species + Month, data = dmerge, family = quasipoisson(link = "log"))
summary(model1) #inflorescences are significant, month is right on the edge (p = 0.053), species is significant (8.81E-5 Let's keep these though; it matches what we saw in the  plots and what they look like by themselves. 
